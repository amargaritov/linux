...
void thp_resvs_new(struct vm_area_struct *vma)
{
  ...
  if (!my_app)
    goto done;
//
	new = vma->vm_mm->thp_reservations;
//  if (new) {
...
//		list_lru_add(&thp_reservations_lru, &res->lru);
  ...
  // It was this way:
	// gfp = GFP_TRANSHUGE_LIGHT & ~__GFP_COMP; //Artemiy changed
  // page = alloc_hugepage_vma(gfp, vma, haddr, HPAGE_PMD_ORDER); 
  // According to definitions from include/linux/gfp.h
...
//  }
	...
	hash_for_each_safe(vma->thp_reservations->res_hash, i, tmp, res, node) {
      ...
      hash_bucket = hash_index(vma->thp_reservations->res_hash, hstart);
      spin_lock(&(vma->thp_reservations->bucket_hash_locks[hash_bucket]));
			khugepaged_free_reservation(res);
      spin_unlock(&(vma->thp_reservations->bucket_hash_locks[hash_bucket]));
    }
				      ...
				      spinlock_t *ptl)
{
	...
	for (_pte = pte; _pte < pte + RESERV_NR; //Artemiy changed 
	...
	prep_compound_page(head, RESERV_ORDER); //Artemiy changed
//	prep_transhuge_page(head); //Artemiy changed
}
				   ...
				   struct thp_reservation *res)
{
	 ...
	 */
  //Artemiy added: if we are going to collapse the page, then release all reserved pages 
  // res always comes 0
  res = 1;
	if (res) {
//		new_page = res->page; //Artemiy changed
//		vma = res->vma; //Artemiy changed
//		is_reserved = true; //Artemiy changed
    int j = 0;
    for(j = 0; j < RESERV_GROUP_NR_IN_PMD; j++) {
      res = khugepaged_find_reservation(vma, (address & RESERV_MASK) + j * RESERV_SIZE);
      if (res) {
        __khugepaged_release_reservations(vma, address, RESERV_SIZE);
      }
    }
    res = 0;
  }
